...
public class GenerateSteps {
	...
	public void one_payroll_is_generated_with(Double moneyMonth, Double bonus,
		...
		System.out.println(
				"mio: " + found.trienniumPayment + " | suyo: " + tri);
	...
	public void payrolls_are_generated_with(Integer howMany, Double moneyMonth,
			Double bonus, Double productivity, Double tri, Double tax,
			Double nic) {
		List<PayrollBLDto> founds = findGeneratedPayrolls();
		this.generatedPayslips = founds.size();
	...
	public void workorders_for_the_mechanic_with_date_and_amount(
		...
		VehicleDto vehicle = new VehicleUtil()	.withOwner(client.id)
												.register()
												.get();
		for (int i = 1; i <= howmany; i++) {
			...
			workOrderDto = new WorkOrderUtil()	.forMechanic(m.id)
												...
												.register()
												.get();
			new InterventionUtil()	.withDate(ldt)
									.withMechanic(m.id)
									.withWorkorder(workOrderDto.id)
									.register();
	...
	public void a_contract_in_force_for_the_mechanic_with(String date,
		...
		ContractTypeDto ct = new ContractTypeUtil().findContractType(
				"PERMANENT").get();
		ProfessionalGroupBLDto pg = new ProfessionalGroupUtil().findProfessionalGroup(
				"I").get();
	...
	public void a_contract_terminated_long_ago_with_base_salary(Double salary)
		...
		ProfessionalGroupBLDto pg = (ProfessionalGroupBLDto) ctx.get(
	...
	public void a_contract_type_permanent() {
		ContractTypeUtil util = new ContractTypeUtil().findContractType(
				"PERMANENT");
	...
	public void a_prof_group_1() {
		ProfessionalGroupUtil util = new ProfessionalGroupUtil().findProfessionalGroup(
				"I");
