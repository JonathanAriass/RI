...
public class PayrollUtil {
	...
	public static boolean match(PayrollBLDto pr1, PayrollBLDto pr2) {
		...
		if ((match(pr1.id, pr2.id)) && (match(pr1.contractId, pr2.contractId))
				&& (match(pr1.date, pr2.date))
				&& Math.abs(
						pr1.monthlyWage - pr2.monthlyWage) < Double.MIN_NORMAL
				&& Math.abs(pr1.bonus - pr2.bonus) < Double.MIN_NORMAL
				&& Math.abs(pr1.trienniumPayment
						- pr2.trienniumPayment) < Double.MIN_NORMAL
				&& Math.abs(pr1.productivityBonus
						- pr2.productivityBonus) < Double.MIN_NORMAL
