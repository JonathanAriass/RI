...
public class InvoicingSteps {
	...
	private void createWorkOrderWithState(String state) {
		...
		workorder = new WorkOrderUtil()	.forVehicle(vehicle.id)
										.withState(state)
										.withAmount(randomAmount(leftLimit,
												rightLimit))
										.register()
										.get();
...
//    }
	...
	public void iGetOnlyFinishedWorkorders() {
		assertTrue(
				this.invoicingWorkOrders.size() == this.onlyFinishedWorkorders.size());
		/* ids match */
	...
	public void anInvoiceIsCreated() {
		...
		/* with proper number */
		/* with proper amount */
		System.out.println(found.total + " | " + expectedInvoice.total);
